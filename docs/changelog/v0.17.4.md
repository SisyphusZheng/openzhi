...
decision was sound as a *startup policy* but became a correctness problem when
...
for packages that *do* have validated SSR capability.
...
component *already* has a DSD template (from the parent's `render()` output),
...
#### Shoelace Showcase Color Contrast

**Problem**: Shoelace's default theme colors (Primary blue `#6366f1`, Success
green `#10b981`, Neutral gray `#71717a`) appeared too pale against the docs
site's light background, especially the Neutral variant which nearly blended
into the background.

**Fix** (`www/app/islands/shoelace-showcase.ts`):

Overrode Shoelace's color tokens with higher-contrast values:

```css
--sl-color-primary-600: #4f46e5;  /* deeper indigo */
--sl-color-success-600: #059669;  /* deeper emerald */
--sl-color-neutral-600: #52525b;  /* darker zinc */
```

### Effect

- Vanilla adapter `ssr: false` islands now render on first paint
- Shoelace buttons have visibly improved contrast
...
| `www/app/islands/shoelace-showcase.ts`                    | Higher-contrast Shoelace color tokens                  |
