...
`functions/api/term.ts` 放在仓库根目录（与 `www/` 同级）。Cloudflare Pages 在部署时会自动扫描根目录下的 `functions/` 目录，将其中的文件编译为 serverless functions，无需额外配置。

构建部署后，在 Cloudflare Pages 仪表盘 → 项目 → Functions 中能看到 `api/term` 已激活。

> **注意：** `functions/` 的路径取决于 Cloudflare Pages 项目设置中的"根目录"。如果根目录设为 `src-tmp`，则 `functions/` 必须在 `src-tmp/functions/`。如果根目录设为 `/`，则 `functions/` 必须在仓库根目录。
